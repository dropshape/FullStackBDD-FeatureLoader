...
module.exports = function Loader(pattern, options) {
    ...
    var filePatterns = [];
    var position = 0;
    glob.glob(resolveDirectoryPattern(pattern), options, function (err, value) {
        if (err) {
...
/**
 * If the pattern ends with a forwards or backwards slash then
 * append a wildcard as the user is trying to load all files from
 * a given directory.
 * @param pattern
 * @returns {*}
 */
function resolveDirectoryPattern(pattern) {
    if (pattern.lastIndexOf(path.sep) === pattern.length - 1) {
